"""Tests for the pipeline handoff to backend via Celery."""
...
class TestPipelineHandoff:
    """Tests for pipeline handoff to backend via Celery."""

    def test_main_publishes_crawl_job_after_extract(self, monkeypatch):
        """
        The pipeline publishes the crawl job to Celery after extraction and
        returns True.
        """
        import main
        ...
        monkeypatch.setattr("main.publish_process_crawl_job", mock_publish)
    ...
    def test_main_without_redis_url_raises(self, monkeypatch):
        """
        With no REDIS_URL the pipeline propagates RuntimeError from
        publish_process_crawl_job and returns False.
        """
        import main
        ...
        monkeypatch.setattr("main.publish_process_crawl_job", _raise_runtime)
